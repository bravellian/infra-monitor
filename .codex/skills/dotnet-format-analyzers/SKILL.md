...
Format verification:
```bash
dotnet format infra-monitor.slnx --verify-no-changes
```

Analyzer verification:
```bash
dotnet format analyzers infra-monitor.slnx --verify-no-changes
```
...
bash .codex/skills/dotnet-format-analyzers/scripts/run-format-analyzers.sh infra-monitor.slnx --include src/app/Incursa.Web/Pages/Error.cshtml.cs
```
